...
return {
	...
	dependencies = vim.tbl_extend('force', {
		...
		'saghen/blink.cmp',
	}, not utils.is_nixos() and {
		-- Only include Mason dependencies on non-NixOS systems
	...
	config = function()
				...
				settings = {
					nixd = {
						...
						options = {
							nixos = {
								expr =
								'let flake = builtins.getFlake(toString ./.); in flake.nixosConfigurations.hephaestus.options',
							},
		...
		if use_mason then
			...
			-- Direct LSP setup for NixOS systems
			-- Users need to install LSPs via Nix configuration
